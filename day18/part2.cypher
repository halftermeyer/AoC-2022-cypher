...
MATCH (c:Cube)
WITH DISTINCT x_max, y_max, c.z AS z_max ORDER BY z_max DESC LIMIT 1
UNWIND range(-1, x_max+1) AS x
UNWIND range(-1, y_max+1) AS y
UNWIND range(-1, z_max+1) AS z
MERGE (:Cube {x: x, y: y, z: z});
...
MATCH (c:Cube&Lava) REMOVE c:Cube;

CALL gds.graph.drop('water', false);
...
CALL gds.wcc.write('water', { writeProperty: 'componentId' })
YIELD nodePropertiesWritten, componentCount;

MATCH (c:Cube {x:-1, y:-1, z:-1})
WITH c.componentId AS comp
MATCH (c:Cube WHERE c.componentId <> comp)
SET c:Lava;

MATCH (n:!Lava) DETACH DELETE n;
MATCH (n) SET n:Cube;
MATCH ()-[r]->() DELETE r;
...
RETURN 6*cubes - 2*adjacencies AS `part 2`;

...
// prod is lcm because set of test_div relatively prime
MATCH (m:Monkey)
WITH collect(m.test_div) AS test_vals
WITH reduce(acc=1, v IN test_vals | acc * v) AS prod_test_div
MATCH (m:Monkey)
SET m.prod_test_div = prod_test_div;
...
CALL apoc.periodic.commit('
  ...
  CALL apoc.cypher.run(apoc.text.replace(
    ...
    {})
  YIELD value
  WITH value.new % m.prod_test_div AS new_val, m

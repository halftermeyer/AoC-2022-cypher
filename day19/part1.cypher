...
// heuristics
MATCH (s:Unprocessed WHERE s.minute <= 24)
WITH s, s.blueprint AS blueprint,
s.geodes + s.geodeBot * (24 - s.minute) AS g,
s.obsidian + s.obsidianBot * (24 - s.minute) AS ob,
s.clay + s.clayBot * (24 - s.minute) AS c,
s.ore + s.oreBot * (24 - s.minute) AS or
...
CALL apoc.periodic.iterate(
  'MATCH (s:State&Unprocessed WHERE s.minute <= 24)
  RETURN s',
  'MATCH (a:Action)
...
ORDER BY blueprint, geodes DESC
WITH blueprint, collect(geodes)[0] AS geodes
RETURN sum (blueprint * geodes) AS `part 1`;
